A REFAIRE SUR BASE DES MODIFICATIONS DEPUIS LYON (mai 2017)
...
################################################################################################################
################################################################################################################
# Code for computing the Power and explanatory accuracy (R²adj) of the different spatial weighting matrices    #
# tested above.                                                                                                #
################################################################################################################
################################################################################################################
...
resultsB_FWD <- as.data.frame(matrix(nrow = 23, ncol = 2005))
...
resultsB_FWD[,1] <- c(rep(c("del", "gab", "rel", "mst", "DB"), each = 4),
                      "DB (thresh mst)", "R2adjReal", "pvalReal")
resultsB_FWD[,2] <- c(rep(c("None", "Linear", "Concave-down", "Concave-up"), times = 5),
                      "1-(D/4t)^2", NA, NA)

resultsM_FWD <- as.data.frame(matrix(nrow = 23, ncol = 2005))
colnames(resultsM_FWD) <- c("Matrix B", "Matrix A", "Power", "MedianDeltaR2", "sd DeltaR2",
                            paste("p-val", c(1:1000), sep = ""), paste("deltaR2_", c(1:1000), sep = ""))
resultsM_FWD[,1] <- c(rep(c("del", "gab", "rel", "mst", "DB"), each = 4),
                      "DB (thresh mst)", "R2adjReal", "pvalReal")
resultsM_FWD[,2] <- c(rep(c("None", "Linear", "Concave-down", "Concave-up"), times = 5),
                      "1-(D/4t)^2", NA, NA)

resultsF_FWD <- as.data.frame(matrix(nrow = 23, ncol = 2005))
colnames(resultsF_FWD) <- c("Matrix B", "Matrix A", "Power", "MedianDeltaR2", "sd DeltaR2",
                            paste("p-val", c(1:1000), sep = ""), paste("deltaR2_", c(1:1000), sep = ""))
resultsF_FWD[,1] <- c(rep(c("del", "gab", "rel", "mst", "DB"), each = 4),
                      "DB (thresh mst)", "R2adjReal", "pvalReal")
resultsF_FWD[,2] <- c(rep(c("None", "Linear", "Concave-down", "Concave-up"), times = 5),
                      "1-(D/4t)^2", NA, NA)
...
if(design == "regular"){
    ...
    for(i in 1:nperm){
      ...
      spesimF[[i]] <- (MEM[, 36] * 0.5) + (MEM[, 37] * 1) + (MEM[, 38] * 1) + rnorm(n, mean = 0, sd = 1)
    }
}

# III. Generation of the MEM variables with the fwd selection:
##############################################################
##############################################################
##############################################################
...
# We now use the structured response variable we generated above to retrieve the spatial R2adj:
# *********************************************************************************************
...
for(i in 1:nperm){
  ...
  resultsB_FWD[22, 1005+i] <- R2adj
  resultsB_FWD[23, 5+i] <- lmp(lm)
  ...
  if(anova.cca(rda(Y, Y.del.MEM$best$MEM))$Pr[1] <= 0.05){
    ...
    if(class != "try-error"){
      ...
      resultsB_FWD[1, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsB_FWD[22, 1005+i] 
    } } else{ 
      resultsB_FWD[1, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
  ...
  if(anova.cca(rda(Y, Y.del.MEM.w.f1$best$MEM))$Pr[1] <= 0.05){
    ...
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.del.MEM.w.f1$best$MEM[, c(sign)]
      resultsB_FWD[2, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsB_FWD[2, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsB_FWD[22, 1005+i]
    } } else{ 
      resultsB_FWD[2, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsB_FWD[2, i+1005] <- NA
    }  
  R2adj <- RsquareAdj(rda(Y, Y.del.MEM.w.f2$best$MEM))$adj.r.squared
  if(anova.cca(rda(Y, Y.del.MEM.w.f2$best$MEM))$Pr[1] <= 0.05){
    class <- class(try(fsel <- forward.sel(Y, Y.del.MEM.w.f2$best$MEM, adjR2thresh = R2adj, nperm = 999), TRUE))
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.del.MEM.w.f2$best$MEM[, c(sign)]
      resultsB_FWD[3, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsB_FWD[3, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsB_FWD[22, 1005+i]
    } } else{ 
      resultsB_FWD[3, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsB_FWD[3, i+1005] <- NA
    }  
  R2adj <- RsquareAdj(rda(Y, Y.del.MEM.w.f3$best$MEM))$adj.r.squared
  if(anova.cca(rda(Y, Y.del.MEM.w.f3$best$MEM))$Pr[1] <= 0.05){
    class <- class(try(fsel <- forward.sel(Y, Y.del.MEM.w.f3$best$MEM, adjR2thresh = R2adj, nperm = 999), TRUE))
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.del.MEM.w.f3$best$MEM[, c(sign)]
      resultsB_FWD[4, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsB_FWD[4, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsB_FWD[22, 1005+i]
    } } else{ 
      resultsB_FWD[4, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsB_FWD[4, i+1005] <- NA
    }  
  ...
  if(anova.cca(rda(Y, Y.gab.MEM$best$MEM))$Pr[1] <= 0.05){
    ...
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.gab.MEM$best$MEM[, c(sign)]
      resultsB_FWD[5, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsB_FWD[5, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsB_FWD[22, 1005+i]
    } } else{ 
      resultsB_FWD[5, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsB_FWD[5, i+1005] <- NA
    }  
  R2adj <- RsquareAdj(rda(Y, Y.gab.MEM.w.f1$best$MEM))$adj.r.squared
  if(anova.cca(rda(Y, Y.gab.MEM.w.f1$best$MEM))$Pr[1] <= 0.05){
    class <- class(try(fsel <- forward.sel(Y, Y.gab.MEM.w.f1$best$MEM, adjR2thresh = R2adj, nperm = 999), TRUE))
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.gab.MEM.w.f1$best$MEM[, c(sign)]
      resultsB_FWD[6, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsB_FWD[6, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsB_FWD[22, 1005+i]
    } } else{ 
      resultsB_FWD[6, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsB_FWD[6, i+1005] <- NA
    }  
  R2adj <- RsquareAdj(rda(Y, Y.gab.MEM.w.f2$best$MEM))$adj.r.squared
  if(anova.cca(rda(Y, Y.gab.MEM.w.f2$best$MEM))$Pr[1] <= 0.05){
    class <- class(try(fsel <- forward.sel(Y, Y.gab.MEM.w.f2$best$MEM, adjR2thresh = R2adj, nperm = 999), TRUE))
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.gab.MEM.w.f2$best$MEM[, c(sign)]
      resultsB_FWD[7, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsB_FWD[7, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsB_FWD[22, 1005+i]
    } } else{ 
      resultsB_FWD[7, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsB_FWD[7, i+1005] <- NA
    }  
  R2adj <- RsquareAdj(rda(Y, Y.gab.MEM.w.f3$best$MEM))$adj.r.squared
  if(anova.cca(rda(Y, Y.gab.MEM.w.f3$best$MEM))$Pr[1] <= 0.05){
    class <- class(try(fsel <- forward.sel(Y, Y.gab.MEM.w.f3$best$MEM, adjR2thresh = R2adj, nperm = 999), TRUE))
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.gab.MEM.w.f3$best$MEM[, c(sign)]
      resultsB_FWD[8, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsB_FWD[8, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsB_FWD[22, 1005+i]
    } } else{ 
      resultsB_FWD[8, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsB_FWD[8, i+1005] <- NA
    }  
  ...
  if(anova.cca(rda(Y, Y.rel.MEM$best$MEM))$Pr[1] <= 0.05){
    ...
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.rel.MEM$best$MEM[, c(sign)]
      resultsB_FWD[9, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsB_FWD[9, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsB_FWD[22, 1005+i]
    } } else{ 
      resultsB_FWD[9, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsB_FWD[9, i+1005] <- NA
    }  
  R2adj <- RsquareAdj(rda(Y, Y.rel.MEM.w.f1$best$MEM))$adj.r.squared
  if(anova.cca(rda(Y, Y.rel.MEM.w.f1$best$MEM))$Pr[1] <= 0.05){
    class <- class(try(fsel <- forward.sel(Y, Y.rel.MEM.w.f1$best$MEM, adjR2thresh = R2adj, nperm = 999), TRUE))
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.rel.MEM.w.f1$best$MEM[, c(sign)]
      resultsB_FWD[10, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsB_FWD[10, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsB_FWD[22, 1005+i]
    } } else{ 
      resultsB_FWD[10, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsB_FWD[10, i+1005] <- NA
    }  
  R2adj <- RsquareAdj(rda(Y, Y.rel.MEM.w.f2$best$MEM))$adj.r.squared
  if(anova.cca(rda(Y, Y.rel.MEM.w.f2$best$MEM))$Pr[1] <= 0.05){
    class <- class(try(fsel <- forward.sel(Y, Y.rel.MEM.w.f2$best$MEM, adjR2thresh = R2adj, nperm = 999), TRUE))
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.rel.MEM.w.f2$best$MEM[, c(sign)]
      resultsB_FWD[11, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsB_FWD[11, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsB_FWD[22, 1005+i]
    } } else{ 
      resultsB_FWD[11, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsB_FWD[11, i+1005] <- NA
    }  
  R2adj <- RsquareAdj(rda(Y, Y.rel.MEM.w.f3$best$MEM))$adj.r.squared
  if(anova.cca(rda(Y, Y.rel.MEM.w.f3$best$MEM))$Pr[1] <= 0.05){
    class <- class(try(fsel <- forward.sel(Y, Y.rel.MEM.w.f3$best$MEM, adjR2thresh = R2adj, nperm = 999), TRUE))
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.rel.MEM.w.f3$best$MEM[, c(sign)]
      resultsB_FWD[12, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsB_FWD[12, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsB_FWD[22, 1005+i]
    } } else{ 
      resultsB_FWD[12, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsB_FWD[12, i+1005] <- NA
    }  
  ...
  if(anova.cca(rda(Y, Y.mst.MEM$best$MEM))$Pr[1] <= 0.05){
    ...
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.mst.MEM$best$MEM[, c(sign)]
      resultsB_FWD[13, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsB_FWD[13, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsB_FWD[22, 1005+i]
    } } else{ 
      resultsB_FWD[13, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsB_FWD[13, i+1005] <- NA
    }  
  R2adj <- RsquareAdj(rda(Y, Y.mst.MEM.w.f1$best$MEM))$adj.r.squared
  if(anova.cca(rda(Y, Y.mst.MEM.w.f1$best$MEM))$Pr[1] <= 0.05){
    class <- class(try(fsel <- forward.sel(Y, Y.mst.MEM.w.f1$best$MEM, adjR2thresh = R2adj, nperm = 999), TRUE))
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.mst.MEM.w.f1$best$MEM[, c(sign)]
      resultsB_FWD[14, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsB_FWD[14, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsB_FWD[22, 1005+i]
    } } else{ 
      resultsB_FWD[14, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsB_FWD[14, i+1005] <- NA
    }  
  R2adj <- RsquareAdj(rda(Y, Y.mst.MEM.w.f2$best$MEM))$adj.r.squared
  if(anova.cca(rda(Y, Y.mst.MEM.w.f2$best$MEM))$Pr[1] <= 0.05){
    class <- class(try(fsel <- forward.sel(Y, Y.mst.MEM.w.f2$best$MEM, adjR2thresh = R2adj, nperm = 999), TRUE))
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.mst.MEM.w.f2$best$MEM[, c(sign)]
      resultsB_FWD[15, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsB_FWD[15, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsB_FWD[22, 1005+i]
    } } else{ 
      resultsB_FWD[15, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsB_FWD[15, i+1005] <- NA
    }  
  R2adj <- RsquareAdj(rda(Y, Y.mst.MEM.w.f3$best$MEM))$adj.r.squared
  if(anova.cca(rda(Y, Y.mst.MEM.w.f3$best$MEM))$Pr[1] <= 0.05){
    class <- class(try(fsel <- forward.sel(Y, Y.mst.MEM.w.f3$best$MEM, adjR2thresh = R2adj, nperm = 999), TRUE))
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.mst.MEM.w.f3$best$MEM[, c(sign)]
      resultsB_FWD[16, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsB_FWD[16, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsB_FWD[22, 1005+i]
    } } else{ 
      resultsB_FWD[16, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsB_FWD[16, i+1005] <- NA
    }  
  ...
  if(anova.cca(rda(Y, Y.list10dnn.MEM$best$MEM))$Pr[1] <= 0.05){
    ...
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.list10dnn.MEM$best$MEM[, c(sign)]
      resultsB_FWD[17, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsB_FWD[17, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsB_FWD[22, 1005+i]
    } } else{ 
      resultsB_FWD[17, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsB_FWD[17, i+1005] <- NA
    }  
  ...
  if(anova.cca(rda(Y, Y.list10dnn.MEM.w.f1$best$MEM))$Pr[1] <= 0.05){
    ...
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.list10dnn.MEM.w.f1$best$MEM[, c(sign)]
      resultsB_FWD[18, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsB_FWD[18, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsB_FWD[22, 1005+i]
    } } else{ 
      resultsB_FWD[18, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsB_FWD[18, i+1005] <- NA
    }  
  ...
  if(anova.cca(rda(Y, Y.list10dnn.MEM.w.f2$best$MEM))$Pr[1] <= 0.05){
    ...
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.list10dnn.MEM.w.f2$best$MEM[, c(sign)]
      resultsB_FWD[19, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsB_FWD[19, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsB_FWD[22, 1005+i]
    } } else{ 
      resultsB_FWD[19, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsB_FWD[19, i+1005] <- NA
    }  
  ...
  if(anova.cca(rda(Y, Y.list10dnn.MEM.w.f3$best$MEM))$Pr[1] <= 0.05){
    ...
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.list10dnn.MEM.w.f3$best$MEM[, c(sign)]
      resultsB_FWD[20, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsB_FWD[20, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsB_FWD[22, 1005+i]
    } } else{ 
      resultsB_FWD[20, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsB_FWD[20, i+1005] <- NA
    }  
  ...
  if(anova.cca(rda(Y, Y.list10dnn.MEM.w.f4$best$MEM))$Pr[1] <= 0.05){
    ...
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.list10dnn.MEM.w.f4$best$MEM[, c(sign)]
      resultsB_FWD[21, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsB_FWD[21, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsB_FWD[22, 1005+i]
    } } else{ 
      resultsB_FWD[21, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsB_FWD[21, i+1005] <- NA
    }  
}   # End of the 'for' simulation loop.
...
for(i in 1:nrow(resultsB_FWD)){
  resultsB_FWD[i, 3] <- length(which(resultsB_FWD[i, c(6:(nperm + 5))] <= 0.05)) / nperm
  resultsB_FWD[i, 4] <- median(na.omit(as.numeric(resultsB_FWD[i, c(1006:(nperm + 1005))])))
  resultsB_FWD[i, 5] <- sd(na.omit(as.numeric(resultsB_FWD[i, c(1006:(nperm + 1005))])))
}
...
for(i in 1:nperm){
  ...
  lm <- lm(Y ~ ., data = x)
  R2adj <- summary(lm)$adj.r.squared
  resultsM_FWD[22, 1005+i] <- R2adj
  resultsM_FWD[23, 5+i] <- lmp(lm)
  ...
  if(anova.cca(rda(Y, Y.del.MEM$best$MEM))$Pr[1] <= 0.05){
    ...
    if(class != "try-error"){
      ...
      resultsM_FWD[1, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsM_FWD[22, 1005+i] 
    } } else{ 
      resultsM_FWD[1, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
  ...
  if(anova.cca(rda(Y, Y.del.MEM.w.f1$best$MEM))$Pr[1] <= 0.05){
    ...
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.del.MEM.w.f1$best$MEM[, c(sign)]
      resultsM_FWD[2, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsM_FWD[2, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsM_FWD[22, 1005+i]
    } } else{ 
      resultsM_FWD[2, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsM_FWD[2, i+1005] <- NA
    }  
  R2adj <- RsquareAdj(rda(Y, Y.del.MEM.w.f2$best$MEM))$adj.r.squared
  if(anova.cca(rda(Y, Y.del.MEM.w.f2$best$MEM))$Pr[1] <= 0.05){
    class <- class(try(fsel <- forward.sel(Y, Y.del.MEM.w.f2$best$MEM, adjR2thresh = R2adj, nperm = 999), TRUE))
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.del.MEM.w.f2$best$MEM[, c(sign)]
      resultsM_FWD[3, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsM_FWD[3, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsM_FWD[22, 1005+i]
    } } else{ 
      resultsM_FWD[3, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsM_FWD[3, i+1005] <- NA
    }  
  R2adj <- RsquareAdj(rda(Y, Y.del.MEM.w.f3$best$MEM))$adj.r.squared
  if(anova.cca(rda(Y, Y.del.MEM.w.f3$best$MEM))$Pr[1] <= 0.05){
    class <- class(try(fsel <- forward.sel(Y, Y.del.MEM.w.f3$best$MEM, adjR2thresh = R2adj, nperm = 999), TRUE))
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.del.MEM.w.f3$best$MEM[, c(sign)]
      resultsM_FWD[4, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsM_FWD[4, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsM_FWD[22, 1005+i]
    } } else{ 
      resultsM_FWD[4, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsM_FWD[4, i+1005] <- NA
    }  
  ...
  if(anova.cca(rda(Y, Y.gab.MEM$best$MEM))$Pr[1] <= 0.05){
    ...
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.gab.MEM$best$MEM[, c(sign)]
      resultsM_FWD[5, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsM_FWD[5, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsM_FWD[22, 1005+i]
    } } else{ 
      resultsM_FWD[5, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsM_FWD[5, i+1005] <- NA
    }  
  R2adj <- RsquareAdj(rda(Y, Y.gab.MEM.w.f1$best$MEM))$adj.r.squared
  if(anova.cca(rda(Y, Y.gab.MEM.w.f1$best$MEM))$Pr[1] <= 0.05){
    class <- class(try(fsel <- forward.sel(Y, Y.gab.MEM.w.f1$best$MEM, adjR2thresh = R2adj, nperm = 999), TRUE))
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.gab.MEM.w.f1$best$MEM[, c(sign)]
      resultsM_FWD[6, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsM_FWD[6, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsM_FWD[22, 1005+i]
    } } else{ 
      resultsM_FWD[6, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsM_FWD[6, i+1005] <- NA
    }  
  R2adj <- RsquareAdj(rda(Y, Y.gab.MEM.w.f2$best$MEM))$adj.r.squared
  if(anova.cca(rda(Y, Y.gab.MEM.w.f2$best$MEM))$Pr[1] <= 0.05){
    class <- class(try(fsel <- forward.sel(Y, Y.gab.MEM.w.f2$best$MEM, adjR2thresh = R2adj, nperm = 999), TRUE))
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.gab.MEM.w.f2$best$MEM[, c(sign)]
      resultsM_FWD[7, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsM_FWD[7, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsM_FWD[22, 1005+i]
    } } else{ 
      resultsM_FWD[7, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsM_FWD[7, i+1005] <- NA
    }  
  R2adj <- RsquareAdj(rda(Y, Y.gab.MEM.w.f3$best$MEM))$adj.r.squared
  if(anova.cca(rda(Y, Y.gab.MEM.w.f3$best$MEM))$Pr[1] <= 0.05){
    class <- class(try(fsel <- forward.sel(Y, Y.gab.MEM.w.f3$best$MEM, adjR2thresh = R2adj, nperm = 999), TRUE))
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.gab.MEM.w.f3$best$MEM[, c(sign)]
      resultsM_FWD[8, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsM_FWD[8, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsM_FWD[22, 1005+i]
    } } else{ 
      resultsM_FWD[8, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsM_FWD[8, i+1005] <- NA
    }  
  ...
  if(anova.cca(rda(Y, Y.rel.MEM$best$MEM))$Pr[1] <= 0.05){
    ...
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.rel.MEM$best$MEM[, c(sign)]
      resultsM_FWD[9, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsM_FWD[9, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsM_FWD[22, 1005+i]
    } } else{ 
      resultsM_FWD[9, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsM_FWD[9, i+1005] <- NA
    }  
  R2adj <- RsquareAdj(rda(Y, Y.rel.MEM.w.f1$best$MEM))$adj.r.squared
  if(anova.cca(rda(Y, Y.rel.MEM.w.f1$best$MEM))$Pr[1] <= 0.05){
    class <- class(try(fsel <- forward.sel(Y, Y.rel.MEM.w.f1$best$MEM, adjR2thresh = R2adj, nperm = 999), TRUE))
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.rel.MEM.w.f1$best$MEM[, c(sign)]
      resultsM_FWD[10, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsM_FWD[10, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsM_FWD[22, 1005+i]
    } } else{ 
      resultsM_FWD[10, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsM_FWD[10, i+1005] <- NA
    }  
  R2adj <- RsquareAdj(rda(Y, Y.rel.MEM.w.f2$best$MEM))$adj.r.squared
  if(anova.cca(rda(Y, Y.rel.MEM.w.f2$best$MEM))$Pr[1] <= 0.05){
    class <- class(try(fsel <- forward.sel(Y, Y.rel.MEM.w.f2$best$MEM, adjR2thresh = R2adj, nperm = 999), TRUE))
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.rel.MEM.w.f2$best$MEM[, c(sign)]
      resultsM_FWD[11, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsM_FWD[11, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsM_FWD[22, 1005+i]
    } } else{ 
      resultsM_FWD[11, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsM_FWD[11, i+1005] <- NA
    }  
  R2adj <- RsquareAdj(rda(Y, Y.rel.MEM.w.f3$best$MEM))$adj.r.squared
  if(anova.cca(rda(Y, Y.rel.MEM.w.f3$best$MEM))$Pr[1] <= 0.05){
    class <- class(try(fsel <- forward.sel(Y, Y.rel.MEM.w.f3$best$MEM, adjR2thresh = R2adj, nperm = 999), TRUE))
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.rel.MEM.w.f3$best$MEM[, c(sign)]
      resultsM_FWD[12, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsM_FWD[12, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsM_FWD[22, 1005+i]
    } } else{ 
      resultsM_FWD[12, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsM_FWD[12, i+1005] <- NA
    }  
  ...
  if(anova.cca(rda(Y, Y.mst.MEM$best$MEM))$Pr[1] <= 0.05){
    ...
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.mst.MEM$best$MEM[, c(sign)]
      resultsM_FWD[13, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsM_FWD[13, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsM_FWD[22, 1005+i]
    } } else{ 
      resultsM_FWD[13, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsM_FWD[13, i+1005] <- NA
    }  
  R2adj <- RsquareAdj(rda(Y, Y.mst.MEM.w.f1$best$MEM))$adj.r.squared
  if(anova.cca(rda(Y, Y.mst.MEM.w.f1$best$MEM))$Pr[1] <= 0.05){
    class <- class(try(fsel <- forward.sel(Y, Y.mst.MEM.w.f1$best$MEM, adjR2thresh = R2adj, nperm = 999), TRUE))
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.mst.MEM.w.f1$best$MEM[, c(sign)]
      resultsM_FWD[14, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsM_FWD[14, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsM_FWD[22, 1005+i]
    } } else{ 
      resultsM_FWD[14, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsM_FWD[14, i+1005] <- NA
    }  
  R2adj <- RsquareAdj(rda(Y, Y.mst.MEM.w.f2$best$MEM))$adj.r.squared
  if(anova.cca(rda(Y, Y.mst.MEM.w.f2$best$MEM))$Pr[1] <= 0.05){
    class <- class(try(fsel <- forward.sel(Y, Y.mst.MEM.w.f2$best$MEM, adjR2thresh = R2adj, nperm = 999), TRUE))
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.mst.MEM.w.f2$best$MEM[, c(sign)]
      resultsM_FWD[15, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsM_FWD[15, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsM_FWD[22, 1005+i]
    } } else{ 
      resultsM_FWD[15, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsM_FWD[15, i+1005] <- NA
    }  
  R2adj <- RsquareAdj(rda(Y, Y.mst.MEM.w.f3$best$MEM))$adj.r.squared
  if(anova.cca(rda(Y, Y.mst.MEM.w.f3$best$MEM))$Pr[1] <= 0.05){
    class <- class(try(fsel <- forward.sel(Y, Y.mst.MEM.w.f3$best$MEM, adjR2thresh = R2adj, nperm = 999), TRUE))
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.mst.MEM.w.f3$best$MEM[, c(sign)]
      resultsM_FWD[16, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsM_FWD[16, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsM_FWD[22, 1005+i]
    } } else{ 
      resultsM_FWD[16, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsM_FWD[16, i+1005] <- NA
    }  
  ...
  if(anova.cca(rda(Y, Y.list10dnn.MEM$best$MEM))$Pr[1] <= 0.05){
    ...
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.list10dnn.MEM$best$MEM[, c(sign)]
      resultsM_FWD[17, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsM_FWD[17, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsM_FWD[22, 1005+i]
    } } else{ 
      resultsM_FWD[17, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsM_FWD[17, i+1005] <- NA
    }  
  ...
  if(anova.cca(rda(Y, Y.list10dnn.MEM.w.f1$best$MEM))$Pr[1] <= 0.05){
    ...
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.list10dnn.MEM.w.f1$best$MEM[, c(sign)]
      resultsM_FWD[18, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsM_FWD[18, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsM_FWD[22, 1005+i]
    } } else{ 
      resultsM_FWD[18, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsM_FWD[18, i+1005] <- NA
    }  
  ...
  if(anova.cca(rda(Y, Y.list10dnn.MEM.w.f2$best$MEM))$Pr[1] <= 0.05){
    ...
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.list10dnn.MEM.w.f2$best$MEM[, c(sign)]
      resultsM_FWD[19, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsM_FWD[19, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsM_FWD[22, 1005+i]
    } } else{ 
      resultsM_FWD[19, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsM_FWD[19, i+1005] <- NA
    }  
  ...
  if(anova.cca(rda(Y, Y.list10dnn.MEM.w.f3$best$MEM))$Pr[1] <= 0.05){
    ...
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.list10dnn.MEM.w.f3$best$MEM[, c(sign)]
      resultsM_FWD[20, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsM_FWD[20, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsM_FWD[22, 1005+i]
    } } else{ 
      resultsM_FWD[20, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsM_FWD[20, i+1005] <- NA
    }  
  ...
  if(anova.cca(rda(Y, Y.list10dnn.MEM.w.f4$best$MEM))$Pr[1] <= 0.05){
    ...
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.list10dnn.MEM.w.f4$best$MEM[, c(sign)]
      resultsM_FWD[21, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsM_FWD[21, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsM_FWD[22, 1005+i]
    } } else{ 
      resultsM_FWD[21, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsM_FWD[21, i+1005] <- NA
    }  
}   # End of the 'for' simulation loop.
...
for(i in 1:nrow(resultsM_FWD)){
  resultsM_FWD[i, 3] <- length(which(resultsM_FWD[i, c(6:(nperm + 5))] <= 0.05)) / nperm
  resultsM_FWD[i, 4] <- median(na.omit(as.numeric(resultsM_FWD[i, c(1006:(nperm + 1005))])))
  resultsM_FWD[i, 5] <- sd(na.omit(as.numeric(resultsM_FWD[i, c(1006:(nperm + 1005))])))
}
...
if(design == "regular") x <- MEM[, 56:58] else x <- MEM[, 36:38]
...
for(i in 1:nperm){
  ...
  lm <- lm(Y ~ ., data = x)
  R2adj <- summary(lm)$adj.r.squared
  resultsF_FWD[22, 1005+i] <- R2adj
  resultsF_FWD[23, 5+i] <- lmp(lm)
  ...
  if(anova.cca(rda(Y, Y.del.MEM$best$MEM))$Pr[1] <= 0.05){
    ...
    if(class != "try-error"){
      ...
      resultsF_FWD[1, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsF_FWD[22, 1005+i] 
    } } else{ 
      resultsF_FWD[1, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
  ...
  if(anova.cca(rda(Y, Y.del.MEM.w.f1$best$MEM))$Pr[1] <= 0.05){
    ...
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.del.MEM.w.f1$best$MEM[, c(sign)]
      resultsF_FWD[2, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsF_FWD[2, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsF_FWD[22, 1005+i]
    } } else{ 
      resultsF_FWD[2, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsF_FWD[2, i+1005] <- NA
    }  
  R2adj <- RsquareAdj(rda(Y, Y.del.MEM.w.f2$best$MEM))$adj.r.squared
  if(anova.cca(rda(Y, Y.del.MEM.w.f2$best$MEM))$Pr[1] <= 0.05){
    class <- class(try(fsel <- forward.sel(Y, Y.del.MEM.w.f2$best$MEM, adjR2thresh = R2adj, nperm = 999), TRUE))
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.del.MEM.w.f2$best$MEM[, c(sign)]
      resultsF_FWD[3, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsF_FWD[3, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsF_FWD[22, 1005+i]
    } } else{ 
      resultsF_FWD[3, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsF_FWD[3, i+1005] <- NA
    }  
  R2adj <- RsquareAdj(rda(Y, Y.del.MEM.w.f3$best$MEM))$adj.r.squared
  if(anova.cca(rda(Y, Y.del.MEM.w.f3$best$MEM))$Pr[1] <= 0.05){
    class <- class(try(fsel <- forward.sel(Y, Y.del.MEM.w.f3$best$MEM, adjR2thresh = R2adj, nperm = 999), TRUE))
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.del.MEM.w.f3$best$MEM[, c(sign)]
      resultsF_FWD[4, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsF_FWD[4, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsF_FWD[22, 1005+i]
    } } else{ 
      resultsF_FWD[4, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsF_FWD[4, i+1005] <- NA
    }  
  ...
  if(anova.cca(rda(Y, Y.gab.MEM$best$MEM))$Pr[1] <= 0.05){
    ...
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.gab.MEM$best$MEM[, c(sign)]
      resultsF_FWD[5, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsF_FWD[5, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsF_FWD[22, 1005+i]
    } } else{ 
      resultsF_FWD[5, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsF_FWD[5, i+1005] <- NA
    }  
  R2adj <- RsquareAdj(rda(Y, Y.gab.MEM.w.f1$best$MEM))$adj.r.squared
  if(anova.cca(rda(Y, Y.gab.MEM.w.f1$best$MEM))$Pr[1] <= 0.05){
    class <- class(try(fsel <- forward.sel(Y, Y.gab.MEM.w.f1$best$MEM, adjR2thresh = R2adj, nperm = 999), TRUE))
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.gab.MEM.w.f1$best$MEM[, c(sign)]
      resultsF_FWD[6, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsF_FWD[6, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsF_FWD[22, 1005+i]
    } } else{ 
      resultsF_FWD[6, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsF_FWD[6, i+1005] <- NA
    }  
  R2adj <- RsquareAdj(rda(Y, Y.gab.MEM.w.f2$best$MEM))$adj.r.squared
  if(anova.cca(rda(Y, Y.gab.MEM.w.f2$best$MEM))$Pr[1] <= 0.05){
    class <- class(try(fsel <- forward.sel(Y, Y.gab.MEM.w.f2$best$MEM, adjR2thresh = R2adj, nperm = 999), TRUE))
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.gab.MEM.w.f2$best$MEM[, c(sign)]
      resultsF_FWD[7, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsF_FWD[7, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsF_FWD[22, 1005+i]
    } } else{ 
      resultsF_FWD[7, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsF_FWD[7, i+1005] <- NA
    }  
  R2adj <- RsquareAdj(rda(Y, Y.gab.MEM.w.f3$best$MEM))$adj.r.squared
  if(anova.cca(rda(Y, Y.gab.MEM.w.f3$best$MEM))$Pr[1] <= 0.05){
    class <- class(try(fsel <- forward.sel(Y, Y.gab.MEM.w.f3$best$MEM, adjR2thresh = R2adj, nperm = 999), TRUE))
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.gab.MEM.w.f3$best$MEM[, c(sign)]
      resultsF_FWD[8, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsF_FWD[8, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsF_FWD[22, 1005+i]
    } } else{ 
      resultsF_FWD[8, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsF_FWD[8, i+1005] <- NA
    }  
  ...
  if(anova.cca(rda(Y, Y.rel.MEM$best$MEM))$Pr[1] <= 0.05){
    ...
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.rel.MEM$best$MEM[, c(sign)]
      resultsF_FWD[9, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsF_FWD[9, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsF_FWD[22, 1005+i]
    } } else{ 
      resultsF_FWD[9, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsF_FWD[9, i+1005] <- NA
    }  
  R2adj <- RsquareAdj(rda(Y, Y.rel.MEM.w.f1$best$MEM))$adj.r.squared
  if(anova.cca(rda(Y, Y.rel.MEM.w.f1$best$MEM))$Pr[1] <= 0.05){
    class <- class(try(fsel <- forward.sel(Y, Y.rel.MEM.w.f1$best$MEM, adjR2thresh = R2adj, nperm = 999), TRUE))
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.rel.MEM.w.f1$best$MEM[, c(sign)]
      resultsF_FWD[10, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsF_FWD[10, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsF_FWD[22, 1005+i]
    } } else{ 
      resultsF_FWD[10, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsF_FWD[10, i+1005] <- NA
    }  
  R2adj <- RsquareAdj(rda(Y, Y.rel.MEM.w.f2$best$MEM))$adj.r.squared
  if(anova.cca(rda(Y, Y.rel.MEM.w.f2$best$MEM))$Pr[1] <= 0.05){
    class <- class(try(fsel <- forward.sel(Y, Y.rel.MEM.w.f2$best$MEM, adjR2thresh = R2adj, nperm = 999), TRUE))
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.rel.MEM.w.f2$best$MEM[, c(sign)]
      resultsF_FWD[11, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsF_FWD[11, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsF_FWD[22, 1005+i]
    } } else{ 
      resultsF_FWD[11, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsF_FWD[11, i+1005] <- NA
    }  
  R2adj <- RsquareAdj(rda(Y, Y.rel.MEM.w.f3$best$MEM))$adj.r.squared
  if(anova.cca(rda(Y, Y.rel.MEM.w.f3$best$MEM))$Pr[1] <= 0.05){
    class <- class(try(fsel <- forward.sel(Y, Y.rel.MEM.w.f3$best$MEM, adjR2thresh = R2adj, nperm = 999), TRUE))
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.rel.MEM.w.f3$best$MEM[, c(sign)]
      resultsF_FWD[12, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsF_FWD[12, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsF_FWD[22, 1005+i]
    } } else{ 
      resultsF_FWD[12, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsF_FWD[12, i+1005] <- NA
    }  
  ...
  if(anova.cca(rda(Y, Y.mst.MEM$best$MEM))$Pr[1] <= 0.05){
    ...
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.mst.MEM$best$MEM[, c(sign)]
      resultsF_FWD[13, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsF_FWD[13, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsF_FWD[22, 1005+i]
    } } else{ 
      resultsF_FWD[13, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsF_FWD[13, i+1005] <- NA
    }  
  R2adj <- RsquareAdj(rda(Y, Y.mst.MEM.w.f1$best$MEM))$adj.r.squared
  if(anova.cca(rda(Y, Y.mst.MEM.w.f1$best$MEM))$Pr[1] <= 0.05){
    class <- class(try(fsel <- forward.sel(Y, Y.mst.MEM.w.f1$best$MEM, adjR2thresh = R2adj, nperm = 999), TRUE))
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.mst.MEM.w.f1$best$MEM[, c(sign)]
      resultsF_FWD[14, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsF_FWD[14, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsF_FWD[22, 1005+i]
    } } else{ 
      resultsF_FWD[14, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsF_FWD[14, i+1005] <- NA
    }  
  R2adj <- RsquareAdj(rda(Y, Y.mst.MEM.w.f2$best$MEM))$adj.r.squared
  if(anova.cca(rda(Y, Y.mst.MEM.w.f2$best$MEM))$Pr[1] <= 0.05){
    class <- class(try(fsel <- forward.sel(Y, Y.mst.MEM.w.f2$best$MEM, adjR2thresh = R2adj, nperm = 999), TRUE))
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.mst.MEM.w.f2$best$MEM[, c(sign)]
      resultsF_FWD[15, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsF_FWD[15, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsF_FWD[22, 1005+i]
    } } else{ 
      resultsF_FWD[15, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsF_FWD[15, i+1005] <- NA
    }  
  R2adj <- RsquareAdj(rda(Y, Y.mst.MEM.w.f3$best$MEM))$adj.r.squared
  if(anova.cca(rda(Y, Y.mst.MEM.w.f3$best$MEM))$Pr[1] <= 0.05){
    class <- class(try(fsel <- forward.sel(Y, Y.mst.MEM.w.f3$best$MEM, adjR2thresh = R2adj, nperm = 999), TRUE))
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.mst.MEM.w.f3$best$MEM[, c(sign)]
      resultsF_FWD[16, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsF_FWD[16, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsF_FWD[22, 1005+i]
    } } else{ 
      resultsF_FWD[16, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsF_FWD[16, i+1005] <- NA
    }  
  ...
  if(anova.cca(rda(Y, Y.list10dnn.MEM$best$MEM))$Pr[1] <= 0.05){
    ...
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.list10dnn.MEM$best$MEM[, c(sign)]
      resultsF_FWD[17, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsF_FWD[17, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsF_FWD[22, 1005+i]
    } } else{ 
      resultsF_FWD[17, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsF_FWD[17, i+1005] <- NA
    }  
  ...
  if(anova.cca(rda(Y, Y.list10dnn.MEM.w.f1$best$MEM))$Pr[1] <= 0.05){
    ...
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.list10dnn.MEM.w.f1$best$MEM[, c(sign)]
      resultsF_FWD[18, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsF_FWD[18, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsF_FWD[22, 1005+i]
    } } else{ 
      resultsF_FWD[18, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsF_FWD[18, i+1005] <- NA
    }  
  ...
  if(anova.cca(rda(Y, Y.list10dnn.MEM.w.f2$best$MEM))$Pr[1] <= 0.05){
    ...
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.list10dnn.MEM.w.f2$best$MEM[, c(sign)]
      resultsF_FWD[19, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsF_FWD[19, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsF_FWD[22, 1005+i]
    } } else{ 
      resultsF_FWD[19, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsF_FWD[19, i+1005] <- NA
    }  
  ...
  if(anova.cca(rda(Y, Y.list10dnn.MEM.w.f3$best$MEM))$Pr[1] <= 0.05){
    ...
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.list10dnn.MEM.w.f3$best$MEM[, c(sign)]
      resultsF_FWD[20, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsF_FWD[20, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsF_FWD[22, 1005+i]
    } } else{ 
      resultsF_FWD[20, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsF_FWD[20, i+1005] <- NA
    }  
  ...
  if(anova.cca(rda(Y, Y.list10dnn.MEM.w.f4$best$MEM))$Pr[1] <= 0.05){
    ...
    if(class != "try-error"){
      sign <- sort(fsel$order)
      MEM.FwdSel <- Y.list10dnn.MEM.w.f4$best$MEM[, c(sign)]
      resultsF_FWD[21, i+5] <- as.data.frame(anova.cca(rda(Y, MEM.FwdSel)))$Pr[1]
      resultsF_FWD[21, i+1005] <- RsquareAdj(rda(Y, MEM.FwdSel))$adj.r.squared - resultsF_FWD[22, 1005+i]
    } } else{ 
      resultsF_FWD[21, i+5] <- 1   # p-val made equal to 1 (we did not even enter the fwd sel)
      resultsF_FWD[21, i+1005] <- NA
    }  
}   # End of the 'for' simulation loop.
...
for(i in 1:nrow(resultsF_FWD)){
  resultsF_FWD[i, 3] <- length(which(resultsF_FWD[i, c(6:(nperm + 5))] <= 0.05)) / nperm
  resultsF_FWD[i, 4] <- median(na.omit(as.numeric(resultsF_FWD[i, c(1006:(nperm + 1005))])))
  resultsF_FWD[i, 5] <- sd(na.omit(as.numeric(resultsF_FWD[i, c(1006:(nperm + 1005))])))
}

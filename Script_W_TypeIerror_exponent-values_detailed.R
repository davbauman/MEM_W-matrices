# We need to change the way the matrix C is generated in order to be coherent with the
# simulation design of the power/robustness computation.
...
y <- c(2:10)   # Exponent values tested
...
Y.DBMEM.f2.list <- lapply(y, function(x) test.W.R2(nb= Y.DBMEM, xy = C,
                                                           f = f2, y= x, dmax = dmax,
                                                           MEM.autocor = MEM_model))
Y.DBMEM.f3.list <- lapply(y, function(x) test.W.R2(nb= Y.DBMEM, xy = C,
                                                           f = f3, y= x, dmax = dmax,
                                                           MEM.autocor = MEM_model))
...
nbtest <- length(y)
alpha <- 0.05
if (correction == TRUE) alpha <- 1-((1-alpha)^(1/nbtest))   # Sidak correction
...
for(i in 1:nperm){   
  ...
  if(anova.cca(rda(Y, MEM.best), permutations = 9999)$Pr[1] <= alpha){
